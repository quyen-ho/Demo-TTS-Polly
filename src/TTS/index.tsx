...
export default function TTS() {
  ...
  const defaultText = `The PsyAvatarVoice JS is a virtual assistant designed to integrate with websites. It utilizes a dialog box display with a 3D avatar model that can realistically respond and move like a real assistant.`;
  
  return (
    <>
      <div style={{ position: "relative"}}>
        <canvas id='webgl-canvas' style={{ position: "absolute", top: 0, left: 0}}></canvas>
        <div style={{ position: "absolute", display: "block" }} >
          <textarea id="text-input" style={{ background: 'transparent', color: '#fff', width: '300px', height: '400px', margin: '10px', border: '2px solid #fff', display: 'block'}} name="w3review" rows={4} cols={20} defaultValue={defaultText}>
          </textarea>
          <button id="play" onClick={playAudio} style={{width: '100px', height: '30px'}} disabled={loading} >Start</button>
        </div>
      </div>
    </>
  )
